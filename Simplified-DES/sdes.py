...
def sdes(text: bitarray, key: bitarray, mode) -> bitarray:
    ...
    # Initial permutation
    initialPermuted = bitarray()
    for initPermute in IP:
        initialPermuted.append(text[initPermute])

    half = int(len(initialPermuted) / 2)
    # round #1
    result = round(initialPermuted[half:], keys[mode - 1])  # MODE_ENCRYPT - 1 = 0, MODE_DECRYPT - 1 = 1
    result ^= initialPermuted[:half]
    leftHalf = result

    # round #2
    result = round(result, keys[mode % 2])  # MODE_ENCRYPT % 2 = 1, MODE_DECRYPT % 2 = 0
    result ^= initialPermuted[half:]
    leftHalf = result + leftHalf

    result.clear()
    for inversePermute in IP_1:
        result.append(leftHalf[inversePermute])
    return result
